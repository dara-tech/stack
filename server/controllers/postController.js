...
export const createPost = async (req, res, next) => {
  try {
    const { userId } = req.body.user;
    const { desc, img, title, slug, cat, video } = req.body;

    if (!(desc || img || title || cat || video)) {
      return next(
        "All fields are required. Please enter a description, title, category, and select an image or video."
      );
    }
    ...
    const post = await Posts.create({
      ...
      slug,
      cat,
      video, // Ensure you're including the videoLink field here
    });
